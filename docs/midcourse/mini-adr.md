...
### 6. Explicit `null` for title/status must be rejected, not silently accepted
`TaskUpdate.title` and `TaskUpdate.status` are `Optional`, which is required to let a client **omit** a field and leave it unchanged. However, the first implementation treated an **explicit `null`** the same as an omitted field — both resulted in no error, and `null` silently flowed through `model_copy(update=...)`, which could overwrite a task's title or status with `None` despite `TaskResponse` declaring both as non-nullable. This was found through direct API testing (`PATCH` with `{"title": null}`), not caught by the original test suite. Fixed by having `TaskUpdate`'s validators explicitly reject `None` — this is safe because Pydantic only invokes a field's validator when that field is actually present in the request body (omitted fields skip validation entirely and keep their default), so "omitted" and "explicitly null" remain distinguishable and behave differently, as intended. Added `test_patch_explicit_null_title_rejected_422`, `test_patch_explicit_null_status_rejected_422`, and `test_patch_omitted_title_leaves_it_unchanged` to lock in both halves of this behavior.
